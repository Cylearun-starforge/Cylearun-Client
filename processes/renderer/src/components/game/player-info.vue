...
import { BotDifficulties, BotStyles } from '@cylearun/game/lib/player-type';
import { GameSideNames } from '@cylearun/game/lib/side';
...
const playerAvatar = computed(() => {
  ...
  return rc('user', 'avatar_ai.png');
});

const playerAvatarBox = computed(() => {
  if (player.value.type === 'empty') {
    return rc('user', 'box.png');
  }
...
const playerCard = computed(() => {
  ...
  return rc('user', 'info_card_ai.png');
});
...
const playerTeamTag = computed(() => {
  ...
  return rc('game', `team${player.value.info.team}.png`);
});

const sideImage = computed(() => {
  if (player.value.type === 'empty') {
    return rc('game', 'side_random.png');
  }
  switch (player.value.info.side) {
    case 'fue':
      return rc('game', 'side_fue.png');
    case 'allied':
      return rc('game', 'side_allied.png');
    default:
      return rc('game', 'side_random.png');
  }
});
...
.player-card {
  @rc --bg-url: 'user', 'info_card_box.png';
...
.right-selector-box > div {
  @rc --bg-url: 'game', 'right_selector_box.png';
  background-image: var(--bg-url);
  background-size: 100% 100%;
  width: 36px;
  height: 36px;
