...
import { GameModes } from '@cylearun/game/lib/options';
...
<template>
  <div class="map-list-root flex flex-col">
    <div class="flex mode-filter">
      模式类型:
      <dropdown-selector :candidates="GameModes.map(mode => ({ display: mode, value: mode }))" :value="GameModes[0]" />
    </div>
...
.map-list {
  ...
  width: 100%;
  height: 100%;
  @rc --map-box: 'game', 'map/map_box.png';
  @rc --map-box-bg: 'game', 'map/map_box_bg.png';
