...
export function useFollowMouse(element: Ref<HTMLElement>, config: FollowMouseConfig) {
  ...
  return (event: MouseEvent) => {
    ...
    const positionX = offsetX + moveX + (document.body.clientWidth - element.value.clientWidth) / 2;
    const positionY = offsetY + moveY - (document.body.clientHeight - element.value.clientHeight) / 2;
